...
Job.CfProj <- function(inpVars = list(), dispatcher = character(), projStartDate = as.Date("1900-01-01"), projYears = 50, annualized = TRUE, exportToExcel = FALSE) {
   job <- new(
      Class = "Job.CfProj",
      InpVars = inpVars,
...
setMethod(
   ...
   definition = function(object, jobResult) {
      ...
      cfNames <- names(cfResult)[3:dim(cfResult)[2]]
      ...
      dfCf <- rbind(df, cfResult[(cfResult[,"Timeline"] %in% timeLabel), colnames(cfResult) %in% c("Timeline", cfNames)])
